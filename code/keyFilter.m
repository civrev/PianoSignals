function [ outFreq ] = keyFilter( inputSignal, key )

close all; %Gets rid of other graphs

signal = inputSignal; %Rename signal for testing
xmags = abs(fft(signal)); %get absolute value, or magnitude, fft(signal)
...
hold off;
title('Original Signal FFT with Frequency Responses E3');
xlabel('Frequency in Hz');
xticks(0:200/((col/2)-1):1);
xticklabels(0:200:4000);
ylabel('Magnitude');
